defmodule CodeBreaker.Server do
  ...
  defmodule State do
    defstruct new_game: nil, window: nil, canvas: nil, solution: nil, guesses: [], attempts: 0, status: :running, result: nil
  end
  ...
  def init(_) do
    g = :gs.start()
    window = :gs.window(g, [title: 'CodeBreaker', width: 2 * @frame + @canvas_width + 250, height: 2 * @frame + @canvas_height])
    canvas = create_canvas(window)
    button = :gs.button(window, [x: 2 * @frame + @canvas_width, y: @frame, label: {:text, 'New Game'}])
  ...
  def handle_call({:guess, player, guess}, _from, state) do
    ...
    state = %State{state | guesses: state.guesses ++ [{guess, format_check(check)}], attempts: state.attempts + 1}
    |> update()
    if check == {4, 0} do
      winner = :gs.label(state.window, x: 2 * @frame + @canvas_width, y: @frame + 100, label: {:text, to_charlist("Winner: " <> player)})
      {:reply, :ok, %State{state | status: :ended, result: winner}}
    else
      if state.attempts == 10 do
        result = :gs.label(state.window, x: 2 * @frame + @canvas_width, y: @frame + 100, label: {:text, 'Game ended'})
  ...
  def handle_info({:gs, button, :click, _, _}, %State{new_game: button} = state) do
    ...
    state = %State{state | guesses: [], solution: new_solution(), attempts: 0, status: :running, result: nil}
    |> update()
    {:noreply, state}
  end

  defp create_canvas(window) do
    canvas = :gs.canvas(window, [x: @frame, y: @frame, width: @canvas_width, height: @canvas_height, bg: :white])
    :gs.rectangle(canvas, [coords: [{@frame, @frame}, {@frame + @padding + 5 * (@cell_size + @padding) + 2 * @padding, @frame + @padding + 10 * (@cell_size + @padding)}], fill: @board_color, fg: :black])
    for y <- 0..9 do
      for x <- 0..3 do
        :gs.rectangle(canvas, [coords: [{@frame + @padding + (@cell_size + @padding) * x, @frame + @padding + (@cell_size + @padding) * y}, {@frame + @padding + @cell_size + (@cell_size + @padding) * x, @frame + @padding + @cell_size + (@cell_size + @padding) * y}], fill: @board_color, fg: :black])
      end
      :gs.rectangle(canvas, [coords: [{@frame + 4 * (@padding + @cell_size) + 3 * @padding, @frame + @padding + (@cell_size + @padding) * y}, {@frame + 4 * (@padding + @cell_size) + 3 * @padding + @cell_size, @frame + @padding + (@cell_size + @padding) * y + @cell_size}], fill: @board_color, fg: :black])
    end
    canvas
  end
  ...
  defp draw_check(canvas, col, x, y, color) do
    x1 = @frame + @padding + 4 * (@cell_size + @padding) + 2 * @padding + round(@cell_size / 3 * x - @check_size / 2)
    x2 = @frame + @padding + 4 * (@cell_size + @padding) + 2 * @padding + round(@cell_size / 3 * x + @check_size / 2)
    y1 = @frame + @padding + col * (@cell_size + @padding) + round(@cell_size / 3 * y - @check_size / 2)
    y2 = @frame + @padding + col * (@cell_size + @padding) + round(@cell_size / 3 * y + @check_size / 2)
